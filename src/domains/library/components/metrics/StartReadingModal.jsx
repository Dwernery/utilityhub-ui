...
export const StartReadingModal = ({ setShowAddCurrentlyReading, books }) => {
  ...
  const startReading = (bookId) => {
    ...
    updateBook.mutate(
      ...
      {
        onSuccess: () => {
          addToast(`Started reading "${book?.title || "book"}"`, "success");
  ...
  return (
    <div
      ...
      onClick={() => setShowAddCurrentlyReading(false)}
    >
      <div
        className="bg-white w-full sm:rounded-xl sm:max-w-3xl max-h-[85vh] overflow-y-auto"
