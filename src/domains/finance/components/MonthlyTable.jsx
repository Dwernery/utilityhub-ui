...
export function MonthlyTable({ selectedYear }) {
  ...
  return (
    <div className="w-full bg-white rounded-2xl p-5 shadow-xl border border-slate-200 hover:shadow-2xl transition-all duration-300">
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-200">
              <th className="text-left py-2 px-3 text-slate-500 font-semibold text-sm">
                Month
              </th>
              <th className="text-right py-2 px-3 text-slate-500 font-semibold text-sm">
                Assets
              </th>
              <th className="text-right py-2 px-3 text-slate-500 font-semibold text-sm">
                Liabilities
          ...
          <tbody>
            {monthlyData.map((month, idx) => (
              ...
              >
                <td className="py-2 px-3 text-slate-700 font-semibold group-hover:text-blue-700">
                  {month.month}
                </td>
                ...
                <td
                  className={`py-2 px-3 text-right font-semibold ${month.change >= 0 ? "text-emerald-600" : "text-rose-600"}`}
                >
